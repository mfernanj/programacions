...
async function main() {
  console.log('🌱 Creant programació de Matemàtiques I...')

  // Obtenir IDs
  const nivell = await prisma.nivell.findFirstOrThrow({ where: { codi: '1rBatx' } })
  const materia = await prisma.materia.findFirstOrThrow({ where: { codi: 'MatesI' } })
  const curs = await prisma.cursEscolar.findFirstOrThrow({ where: { actiu: true } })
  const admin = await prisma.usuari.findFirstOrThrow({ where: { rol: 'admin' } })
  ...
  console.log(`✅ Programació creada: ${programacio.id}`)
  ...
  const unitats = [
    ...
    },
    {
      ...
      temporitzacio: '14 sessions',
      objectius: '1. Calcular la derivada d\'una funció en un punt.\n2. Interpretar geomètricament la derivada.\n3. Calcular la recta tangent i normal.\n4. Aplicar la regla de la cadena.\n5. Representar gràficament funcions després d\'un estudi complet.',
